...
// تعريف بيانات المراحل
const stagesData = [
    { id: 1, name: "المرحلة الأولى", levels: 5, completed: false, requiredLevels: 0 },
    { id: 2, name: "المرحلة الثانية", levels: 5, completed: false, requiredLevels: 5 },
    { id: 3, name: "المرحلة الثالثة", levels: 5, completed: false, requiredLevels: 10 },
    { id: 4, name: "المرحلة الرابعة", levels: 4, completed: false, requiredLevels: 15 },
    { id: 5, name: "المرحلة الخامسة", levels: 4, completed: false, requiredLevels: 19 }
];
...
let achievements = [
    ...
    { id: "stage1_complete", title: "بطل المرحلة الأولى", description: "أكمل جميع مستويات المرحلة الأولى", icon: "fa-star", earned: false },
    { id: "stage2_complete", title: "بطل المرحلة الثانية", description: "أكمل جميع مستويات المرحلة الثانية", icon: "fa-star", earned: false },
    { id: "stage3_complete", title: "بطل المرحلة الثالثة", description: "أكمل جميع مستويات المرحلة الثالثة", icon: "fa-star", earned: false },
    { id: "stage4_complete", title: "بطل المرحلة الرابعة", description: "أكمل جميع مستويات المرحلة الرابعة", icon: "fa-star", earned: false },
    { id: "stage5_complete", title: "بطل المرحلة الخامسة", description: "أكمل جميع مستويات المرحلة الخامسة", icon: "fa-star", earned: false },
...
for (let i = 1; i <= 23; i++) {
    ...
    // تحديد المرحلة التي ينتمي إليها المستوى
    let stageId = 1;
    if (i > 19) stageId = 5;
    else if (i > 15) stageId = 4;
    else if (i > 10) stageId = 3;
    else if (i > 5) stageId = 2;
...
function initializeDB() {
    const request = indexedDB.open('CardGameDB', 6); // ترقية الإصدار إلى 6 لإضافة المراحل
        ...
        if (!db.objectStoreNames.contains('stages')) {
            ...
            stagesData.forEach(stage => {
                stagesStore.add({
                    ...
                    requiredLevels: stage.requiredLevels
                });
            });
        }
    };
...
function loadGameData() {
    ...
    stagesRequest.onsuccess = (event) => {
        if (stagesRequest.result && stagesRequest.result.length > 0) {
            stagesData.forEach((stage, index) => {
                const dbStage = stagesRequest.result.find(s => s.id === stage.id);
                if (dbStage) {
                    stagesData[index] = dbStage;
                }
            });
            loadStages();
        }
...
function loadStages() {
    ...
    stagesData.forEach(stage => {
        const stageCard = document.querySelector(`.stage-card[data-stage="${stage.id}"]`);
        if (!stageCard) return;
        
        // حساب عدد المستويات المكتملة في هذه المرحلة
        let completedInStage = 0;
        const startLevel = stage.id === 1 ? 1 : (stage.id - 1) * 5 + 1;
        const endLevel = stage.id === 1 ? 5 : startLevel + stage.levels - 1;
        ...
        for (let i = startLevel; i <= endLevel; i++) {
            ...
            request.onsuccess = (event) => {
                ...
                // تحديث حالة المرحلة بعد التحقق من جميع المستويات
                if (i === endLevel) {
                    const statusElement = stageCard.querySelector('.stage-status');
                    if (completedInStage === stage.levels) {
                        stageCard.classList.remove('locked');
                        stageCard.classList.add('completed');
                        statusElement.innerHTML = `<i class="fas fa-check"></i> مكتملة`;
                        
                        // تحديث حالة المرحلة في قاعدة البيانات
                        if (!stage.completed) {
                            stage.completed = true;
                            const updateTransaction = db.transaction(['stages'], 'readwrite');
                            const stagesStore = updateTransaction.objectStore('stages');
                            stagesStore.put(stage);
                            
                            // فتح الإنجاز الخاص بإكمال المرحلة
                            unlockAchievement(`stage${stage.id}_complete`);
                        }
                    } else if (stage.id > 1) {
                        // التحقق مما إذا كانت المرحلة السابقة مكتملة
                        const prevStage = stagesData.find(s => s.id === stage.id - 1);
                        if (prevStage && prevStage.completed) {
                            stageCard.classList.remove('locked');
                            statusElement.textContent = `${completedInStage}/${stage.levels} مستويات مكتملة`;
                        } else {
                            stageCard.classList.add('locked');
                            statusElement.innerHTML = `<i class="fas fa-lock"></i> مقفلة`;
                        }
                    } else {
                        stageCard.classList.remove('locked');
                        statusElement.textContent = `${completedInStage}/${stage.levels} مستويات مكتملة`;
...
function endGameDueToTime() {
    playSound('error');
    showMessage('انتهى الوقت!', `${userName}، لم تستطع إكمل المستوى في الوقت المحدد`, false);
...
function completeLevel() {
    ...
    levelsStore.put({
        ...
        completed: true,
        points: levelPoints,
        stage: levelsData[currentLevel - 1].stage
    });
...
function setupEventListeners() {
    ...
    // أحداث النقر على بطاقات المراحل
    document.querySelectorAll('.stage-card').forEach(card => {
        card.addEventListener('click', () => {
            if (!card.classList.contains('locked')) {
                ...
                // تصفية المستويات لعرض فقط مستويات هذه المرحلة
                filterLevelsByStage(stageId);
            } else {
                vibrate(100);
...
// تصفية المستويات حسب المرحلة
function filterLevelsByStage(stageId) {
    const levelsContainer = document.getElementById('levels-container');
